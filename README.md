...
| `admin-appearance`   | `registry:component` | The administrator's look from fixed palettes: eighty-six multi-stop gradients and twenty accents, each measured for contrast along the gradient and on hovered controls. `AdminAppearance` is one value — accent, sidebar and sign-in gradients, page backdrops as `{ gradient, soft }`, per-block gradient and heading — that you store; `AppearanceStyle` emits the stylesheet that paints any `data-gradient` element and redefines its tokens, `AppearanceProvider` hands the value to blocks, `Block` is the card of the work area with a corner button in edit mode |
