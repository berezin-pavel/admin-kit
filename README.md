...
sidebar, the header bar and the sign-in screen, a backdrop for every page
and a gradient (and heading treatment) for every block. All of it is picked from fixed
palettes — eighty-seven gradients, each three stops of a single hue,
grouped into ten families (`gradientFamilies`) — neutral, earth and the
eight rainbow hues — and twenty accents shipped by
`admin-appearance` — because every entry has passed the kit's contrast
tests, along the whole gradient and on hovered controls. Both the block
menu's swatch grid and the appearance menu's gradient selects present
the choices grouped by family instead of one flat list.

The palette is where a look starts, not where it ends. Anywhere the
appearance menu picks a colour — the accent, the sidebar, the header
bar, the sign-in screen, the default page backdrop and every per-page
override — "Custom color…" takes a plain `#rrggbb` instead, typed or
picked from the browser's own colour input; a plain light-grey sidebar
needs no gradient. `SurfaceChoice` is `GradientId | CustomColor`, and
`isCustomColor` is the guard that decides which one a stored value is;
`blocks` stay palette-only. Nothing about legibility is left to the
person choosing: for a custom colour the stylesheet measures the text
against it and picks the side that passes 4.5:1, falling back to pure
white or black where the kit's near-white and near-black would not
...
at the right of that bar. Both panels carry the same four pages — the
overview, the orders list, the products catalogue (a tab strip over the
tree, a flat categories list and a flat products list) and the order —
under their own routes; the header's right corner holds search, the bell,
the fullscreen button and the account. Both read the same
...
| `admin-shell`        | `registry:block`     | A persistent full-height frame for the admin panel: an optional header, side navigation (on a wide screen a sidebar, or an icon rail with `collapsed`; on a narrow one no sidebar at all — a compact top bar with the burger, the logo, the app name and the account, and the panel behind the burger holds the sections), a `logo` slot before the app name, a `sidebarProfile` row under it for the account, a `sidebarActions` slot next to the app name, a `sidebarFooter` slot at the bottom, and a work area with its own scrolling. Every sidebar row shares one grid, so the logo, the avatar and the nav icons keep the same centre expanded or collapsed. The `variant` prop picks the look: `card` (the default) keeps the sidebar a padded panel, `flush` glues it to the edge and turns the header into a fixed full-width bar. `sidebarGradient` and `headerGradient` paint those two surfaces with a named gradient of the appearance palette. The header shows the active nav item's icon before the section title. Installed once per project |
| `app-logo`           | `registry:component` | A brand mark for the shell's `logo` slot, so a freshly installed panel isn't nameless in the collapsed rail: a rounded accent square holding a lucide `icon`, or the first letter of `name` when there's no icon yet. Decorative on purpose — the app name stands beside it, so the mark stays out of the accessibility tree. No state, no client boundary                                                        |
| `admin-appearance`   | `registry:component` | The administrator's look from fixed palettes: eighty-seven single-hue gradients in ten families and twenty accents, each measured for contrast along the gradient and on hovered controls — or from a `#rrggbb` colour of your own, whose text colour and destructive pair the stylesheet derives and measures for you. `AdminAppearance` is one value — accent, sidebar, header and sign-in surfaces, page backdrops as `{ gradient, soft }`, per-block gradient and heading — that you store; `AppearanceStyle` emits the stylesheet that paints any `data-gradient` element and redefines its tokens, `AppearanceProvider` hands the value to blocks, `Block` is the card of the work area with a corner button in edit mode, and `appearanceThemes` holds twenty ready presets. A custom colour is emitted per scheme: `customDarkColor` maps a light one into the dark chrome band at its own hue, so the dark theme never shows a white slab |
| `appearance-menu`    | `registry:component` | The footer control that edits the appearance: a two-column grid of twenty theme presets (a pick keeps the per-block and per-page choices), accent swatches, one select each for the sidebar, the header bar, the sign-in screen and the default page backdrop, and a row per page that inherits or overrides. Every backdrop select has a soften checkbox beside it — checked, the backdrop is the gradient's soft tint; unchecked, the gradient itself. Every select and the accent row also take a colour of your own, entered as `#rrggbb` or picked from the browser's colour input. Names of gradients, accents and themes are props, so `locale-ru` localizes them. The popup carries a grip at its top: dragging it moves the popup aside so the panel under it stays visible while colours are picked |
...
| `fullscreen-toggle`  | `registry:component` | A button that puts the whole panel into the browser's full-screen mode and back; it follows the Fullscreen API itself (the state is the browser's, not the consumer's) and renders nothing where full screen is forbidden |
...
| `user-menu`          | `registry:component` | An avatar icon button that opens a dropdown with the user's name, email, and a list of `items` (icon, label, and a `danger` tone for actions like sign-out). Without `avatarUrl` the trigger shows bare initials in the toggles' lettering. `variant="row"` fills the shell's `sidebarProfile` row — avatar, name, email and a chevron in one trigger, which is where identity belongs in a sidebar; `variant="icon"` (the default) is an outline icon button with the avatar inside — the same control as the notifications bell — and suits `sidebarActions`, the header's `actions` slot or `sidebarFooter` |
| `global-search`      | `registry:component` | A ⌘K dialog over the `items` you pass: it filters them on the label and the group name, groups the matches in the order the groups first appear and keeps the ungrouped ones last, then reports the pick as `onSelect(id)` — no fetching and no routing of its own. Up, down and Enter walk the list under a proper combobox/listbox pairing; `open`/`onOpenChange` stay with you, and `hotkey` (on by default) toggles the same handler, ignoring the shortcut while the caret sits in someone else's input. The box hangs below the top edge and keeps one height while you type, a close cross sits beside the input, and closing returns focus to the trigger. The trigger is a search-shaped field with the shortcut in a `kbd`, or `variant="icon"` for a tight header |
| `notifications-menu` | `registry:component` | A bell button with the unread count over it — capped at `9+` in the badge, spelled out in the button's name — opening a dropdown of entries: a title, an optional explanation clamped to two lines, and a preformatted `timestamp` rendered as it arrives. Read state, ordering and the mark-all-read effect stay with you: the item only reports `onSelect(id)` and `onMarkAllRead`. With no entries it shows the caught-up line instead of the list                                                        |
...
| `widget-table`       | `registry:component` | A self-contained table card: the heading is optional, the header has a `toolbar` slot for filters, and the footer has a record count, pagination, and a page-size picker. A column with `sortable` gets a sort button and `aria-sort`; row order and slicing are set by the data owner, the table only reports the choice via `onSortChange` and `onPageChange`. Passing both `selectedKeys` and `onSelectionChange` adds a checkbox column — the header checkbox selects or clears the current page only — and swaps the header's left side for a selection bar with an N-selected count and `selectionActions`. `filtered` swaps the empty state for a filtered-results one with a clear-filters button; `hiddenColumnIds` plus a columns menu hide columns, except those marked `alwaysVisible` and those with no title; `stickyHeader` keeps the header in view while the body scrolls inside its own area, sized by `maxBodyHeight`; `totalCount` and `onSelectAllMatching` let the selection bar offer to extend a full-page selection to every matching record; export is a `selectionAction` the consumer wires up, with the exported `toCsv` helper for the usual case; a column marked `grow` takes all the slack so hiding another column leaves the rest in place, and `fill` stretches the card to its container with the body scrolling under a sticky header; `onRowActivate` turns a row into a click/Enter activation target, skipping clicks that land on an interactive element inside it; `notice` renders a full-width slot above the body, which `page-list` uses to show a state message while keeping the rows on screen |
| `widget-tree-table`  | `registry:component` | A table card whose rows are sections that expand to reveal records, nesting to any depth: one colour stripe per ancestor at the left edge, the section's own stripe carries its icon, and the first column holds the section title as a button with `aria-expanded`. A column pulls a record's value via `cell` and, optionally, a section's via `sectionCell`; expansion is controlled (`expandedIds`/`onExpandedChange`, with `toggleExpandedId` and `collectSectionIds` helpers) and collapsed content is not mounted; `rowActions`/`sectionActions` render a trailing actions column for `row-actions` and the dialogs the consumer opens from them; without sections it shows `state-empty`, `loading` swaps the body for skeletons; `hiddenColumnIds` plus a columns menu, built-in expand-all/collapse-all icons, `filtered`/`onClearFilters`, `onRowActivate` (the record's name becomes a button), `rowCount` + `onLoadMoreRows` for a "Show N more" row, `grow` and `fill` |
...
| `widget-quick-actions` | `registry:component` | A grid of shortcut buttons — `id`, `label`, an optional icon, `onSelect` — with `columns` switching between two (default) and three. No actions is a consumer mistake, not a data state, so it doesn't fall back to `state-empty`; the grid takes arrow keys |
...
| `page-entity`        | `registry:component` | A single-record page: a heading with actions and fields grouped into sections, with the field value being `ReactNode` rather than a string. A section's `columns` (1, 2 or 3) sets how densely its fields sit. The `status` prop swaps the fields for a loading, error, forbidden, or offline state, while the heading stays visible; those three states also take a `description`, and `stateActions` renders actions (a Retry button, say) inside the matching one |
...
| `color-field`        | `registry:component` | A color-picker field for a label, category, or kanban column: the value is HEX `#rrggbb`, with a `presets` palette in the popover, a native picker, and manual HEX entry; presets are a radio group with arrow keys, a pick closes the popover and so does Enter on a complete hex |
| `icon-field`         | `registry:component` | A field that picks an icon by name from the whole lucide set plus lucide lab (one stroke style): a searchable 8-wide grid with arrow keys, English tags from lucide and Russian names from `locale-ru`, loaded only when the popover opens; the value is the icon name, and `resolveIcon(name)` renders it anywhere |
| `text-field`         | `registry:component` | A single-line text input; the `type` prop switches between text, email, password, url and tel. Like every field: label, hint and an `error` that replaces the hint and sets `aria-invalid`; `filter` sanitizes what is typed — `textFilters` ships digits, phone (`phoneFilter("+7 (###) ###-##-##")`), email, slug (kebab-case with Cyrillic transliteration) and code |
...
| `page-tabs`          | `registry:component` | Section navigation inside one page — Overview, History, Settings on a record screen. A tab is drawn like a sidebar nav item, not as a segmented control. Controlled through `value`/`onValueChange`, so the active tab can live in the URL and a tab can navigate instead of switching panels; only the active panel mounts, so a heavy table on a hidden tab never renders. `actions` sits beside the strip, which scrolls horizontally when the labels do not fit; `fill` lets the active panel take the remaining height
| `page-form`          | `registry:component` | A create/edit record page: a heading, sections of consumer-laid-out fields inside a real `form`, and a footer with Cancel and Save. A section's `columns` (2 or 3) turns it into a responsive grid where each field is one cell, and a field asks for the full width with `col-span`. `submitting` disables the buttons and sets `aria-busy`; the `status` prop swaps sections for a state screen while the heading stays, and `stateActions` renders actions inside the error, forbidden, or offline state, each of which also takes a `description`                                                                                                 |
...
| `page-list`          | `registry:component` | A list page: a `page-header` block and a `widget-table` card below it — filters in its header, pagination and a page-size picker in the footer; `header={false}` drops the header block, makes `title` the table's own (hideable) heading and moves `actions` into the table header. Every value is controlled, and pagination isn't rendered without `total`. A filter is `search`, `select`, or `date-range` (rendered via `date-range-field`). The `status` prop swaps the card's body for a state, while the heading and filters stay available. `selectedKeys`, `onSelectionChange`, and `selectionActions` forward straight through to `widget-table`, the same way sorting, pagination and `onRowActivate` do; `toolbar` adds a page-level control next to the filter bar; a filter's reset button compares its `value` against `defaultValue` (empty by default), so a select modeled with a non-empty default, like an "all" option, does not pin the button on screen; `keepRows` keeps the table's rows on screen through an `error` or `offline` status, showing the state message above them instead, and `stateActions` renders actions inside any of the non-ready states; `fill` stretches the page to the work area's bottom |
`widget-table`, `widget-tree-table`, `widget-chart`, `widget-list`,
`widget-activity`, and `widget-donut` — the widgets with data — show
`state-empty` themselves when there's no data: no rows, no sections, no
series, no list items, no entries, or slices that total zero, and the `empty`/`labels.emptyTitle` prop wasn't
...
The `variant` prop chooses between that look (`card`, the default) and
`flush`: there the shell has no padding at all, the sidebar is glued to
the top, the bottom, and the left edge of the screen, and a single
border-r separates it from the work area. With the header on, `flush`
also moves it out of the work column and across the full width of the
screen — a fixed bar whose left cell holds the brand above the sidebar
and lines up with it, while the work area stays the only thing that
scrolls. The sidebar then starts with the account row instead of
repeating the brand.
