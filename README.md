...
overview, the orders list, the products catalogue and the order — under their own routes. Both read the same
...
| `page-list`          | `registry:component` | A list page: a `page-header` block and a `widget-table` card below it — filters in its header, pagination and a page-size picker in the footer; `header={false}` drops the header block, makes `title` the table's own (hideable) heading and moves `actions` into the table header. Every value is controlled, and pagination isn't rendered without `total`. A filter is `search`, `select`, or `date-range` (rendered via `date-range-field`). The `status` prop swaps the card's body for a state, while the heading and filters stay available. `selectedKeys`, `onSelectionChange`, and `selectionActions` forward straight through to `widget-table`, the same way sorting and pagination do                                     |
