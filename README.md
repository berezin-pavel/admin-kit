...
| `widget-table`       | `registry:component` | A self-contained table card: the heading is optional, the header has a `toolbar` slot for filters, and the footer has a record count, pagination, and a page-size picker. A column with `sortable` gets a sort button and `aria-sort`; row order and slicing are set by the data owner, the table only reports the choice via `onSortChange` and `onPageChange`. Passing both `selectedKeys` and `onSelectionChange` adds a checkbox column — the header checkbox selects or clears the current page only — and swaps the header's left side for a selection bar with an N-selected count and `selectionActions`. `filtered` swaps the empty state for a filtered-results one with a clear-filters button; `hiddenColumnIds` plus a columns menu hide columns, except those marked `alwaysVisible`; `stickyHeader` keeps the header in view while the body scrolls inside `maxBodyHeight`; `onExport` hands the current rows to the consumer, with a `toCsv` helper for the usual case |
