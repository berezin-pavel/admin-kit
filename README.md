...
palettes — one hundred fifteen gradients (twenty-nine single-hue three-stop ones plus eighty-six multi-stop), grouped into nine
...
appearance menu's gradient selects present the choices
grouped by family instead of one flat list. A page backdrop is the soft
...
| `admin-appearance`   | `registry:component` | The administrator's look from fixed palettes: one hundred fifteen gradients in nine families and twenty accents, each measured for contrast along the gradient and on hovered controls. `AdminAppearance` is one value — accent, sidebar, header and sign-in gradients, page backdrops as `{ gradient, soft }`, per-block gradient and heading — that you store; `AppearanceStyle` emits the stylesheet that paints any `data-gradient` element and redefines its tokens, `AppearanceProvider` hands the value to blocks, `Block` is the card of the work area with a corner button in edit mode |
| `appearance-menu`    | `registry:component` | The footer control that edits the appearance: accent swatches, one select each for the sidebar, the header bar, the sign-in screen and the default page backdrop, and a row per page that inherits or overrides. Every backdrop select has a soften checkbox beside it — checked, the backdrop is the gradient's soft tint; unchecked, the gradient itself. Nothing is typed in; names of gradients and accents are props, so `locale-ru` localizes them |
