...
| `image-field`        | `registry:component` | A gallery of a record's images: thumbnails in a responsive grid, a drop zone that takes dragged files or opens the picker, reordering by drag (plus move buttons, since drag never reaches a keyboard), a full-size preview dialog, a direct link that opens the original in a new tab, and removal that applies at once. It uploads nothing itself — `onSelect` hands over the picked `File`s and the stored result comes back through `value` as `{id, url, name}`; `multiple={false}` and `maxItems` narrow the set
